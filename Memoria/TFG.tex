...
Cuando alguien piensa en \textit{\Gls{dfa}}, lo más común es pensar en las aplicaciones como Google Authenticator. Lo siguiente que viene a la mente es usar una llave \Gls{USB} como segundo factor de autenticación, pero la realidad es que se pueden usar dispositivos como unos auriculares comprobando que se complete un patrón como por ejemplo insertar y extraer el jack de audio dos veces seguidas, la recepción de una red \textit{\Gls{wifi}} comprobando  el \textit{\Gls{finterprint}} de los dispositivos que emiten una señal \textit{\Gls{wifi}} con una lista de \textit{\Gls{fingerprint}s} acaptados y la proximidad de dispositivos, comprobando el \textit{\Gls{fingerprint}} contra una lista de \textit{\Gls{fingerprint}s} aceptados. Solamente es necesario que el sistema operativo lo reconozca. Llegué a esta conclusión mientras buscaba posibles dispositivos hardware para realizar la autenticación. 
