...
Este es la parte práctica, derivada de la parte teórica de este trabajo. El \gls{script} se he escrito en \gls{bash}. Un lenguaje intérprete de comandos. Esto significa que une distintos programas disponibles, como: \textit{ls, mv, cd, sed} o \textit{cat} para crear un único programa, que cumple una función específica.\par
Se decidió usar \gls{bash} porque está disponible en todos los equipos que usan \gls{acr-pam}, me interesaba aprenderlo inicialmente y la curva de aprendizaje inicial no parecía muy complicada.\par
